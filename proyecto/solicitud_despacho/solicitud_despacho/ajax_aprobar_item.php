	...
	if(!empty($_POST['idMaterial'])):	
		...
		if($_POST['type'] == 'All'):
			...
			while( $rs_pai2 = mysql_fetch_assoc($pai2) ):
				...
				/* Consulta en inventario cantidad y costo unidad */
				$sql_i = "	SELECT cantidad, costo_unidad
							FROM  `inventario` 
							WHERE  `id` = ".$rs_pai2['id_material'];
				$result_i = mysql_query($sql_i); 
				$row_i = mysql_fetch_assoc($result_i);
				
				$cantidadS = $rs_pai2['cantidadc'] - $rs_pai2['cantidade'];
				$cantidadT = $row_i['cantidad'] + $cantidadS; // Cantidad Inv + cantidad Sobro
				
				$costoC = $rs_pai2['costo_unidad'] * $cantidadS;
				...
				$sql = sprintf("UPDATE inventario SET cantidad='%s', costo_unidad='%s' WHERE id=%d;",
					fn_filtro($cantidadT),
					fn_filtro($precioP),
