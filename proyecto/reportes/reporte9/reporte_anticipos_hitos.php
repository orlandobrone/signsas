...
<?		
 ...
 $query = "SELECT *, s.id AS ID, c.nombre AS centro_costo, (SELECT c.nombre 
			  ...
			  FROM anticipos_eliminados AS s 
	...
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)):
        ...
        $pai6 = mysql_query($sql6); 
			...
			$sql7 = "SELECT p.nombre AS nombre, c.nombre AS nomcliente FROM `proyectos` AS p, cliente AS c

				 	 WHERE p.id = ".(int)$items['id_proyecto']." AND p.id_cliente = c.id LIMIT 1";
