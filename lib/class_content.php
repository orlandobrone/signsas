<?php
  ...
  class Content
  {
	  ...
	  public function processPage()
	  {
		  ...
		  if (empty($core->msgs)) {
			  $data = array(
				  ...
				  'custom_bg' => sanitize($_POST['custom_bg'])
			  );
      ...
      public function getPluginLayout($place, $modalias = false)
      {
		  ...
		  } elseif($this->homeid == 0 and $this->is_homemod and preg_match('/index.php/', htmlspecialchars($_SERVER['PHP_SELF']))) {
       ...
       * @return
       */
      public function getPluginLayoutFront($modalias = false)
      {
          global $db, $core;
      ...
      /**
       * Content::searchPluginLayout()
       * 
       * @param bool $position
       * @return
       */
	  public function searchPluginLayout($array, $position)
	  {
		  if($array) {
			  $result = array();
      ...
      public static function fieldSection($section)
      {
          switch($section) {
			  case "profile":
			  return _CFL_SECTION_P;
			  break;

			  case "register":
			  return _CFL_SECTION_R;
			  break;
      ...
      public static function getFieldSection($section = false)
      {
          ...
          $arr = array(
				 'profile' => _CFL_SECTION_P,
				 'register' => _CFL_SECTION_R
		  );
      ...
      public function processField()
      {
		  ...
		  if (empty($_POST['title'.$core->dblang]))
			  $core->msgs['title'] = _CFL_NAME_R;

          if (empty($core->msgs)) {
              $data = array(
					'title'.$core->dblang => sanitize($_POST['title'.$core->dblang]), 
					'tooltip'.$core->dblang => sanitize($_POST['tooltip'.$core->dblang]), 
					'req' => intval($_POST['req']),
					'active' => intval($_POST['active']),
					'type' => sanitize($_POST['type'])
			  );
              if(!$this->id) {
				 $data['name'] = sanitize($_POST['type']) . time();
			  }
			  
              ($this->id) ? $db->update("custom_fields", $data, "id='" . $this->id . "'") : $db->insert("custom_fields", $data);
              $message = ($this->id) ? _CFL_UPDATED : _CFL_ADDED;
	   ...
	   * @return
	   */
	  public function rendertCustomFields($type, $data, $front = false)
	  {
		  global $db, $core;
	
		  $html = '';
		  if ($fdata = $db->fetch_all("SELECT * FROM custom_fields WHERE type = '" . $db->escape($type) . "' AND active = 1 ORDER BY sorting")) {
			  ...
			  foreach ($fdata as $k => $cfrow) {
				  $html .= '<tr>';
				  $html .= '<th>' . $cfrow['title' . $core->dblang] . ':';
				  if ($cfrow['req']) {
					  $html .= ' ' . required();
				  }
				  if ($front and $cfrow['tooltip' . $core->dblang]) {
					  $style = ' style="max-width:95%"';
				  }
				  $html .= '</th>';
				  if ($front) {
					  $dstart = '<div class="placeholder">';
					  $dend = '</div>';
				  }
				  $html .= '<td>' . $dstart . '<input name="custom_' . $cfrow['name'] . '" type="text" ' . $style . ' class="inputbox" size="55" value="' . $value[$k] . '"/>';
				  if ($cfrow['tooltip' . $core->dblang]) {
					  $html .= ' ' . tooltip($cfrow['tooltip' . $core->dblang]);
				  }
