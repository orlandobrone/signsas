<?php
  ...
  class Users
  {
      ...
      private function startSession()
      {
		...
		if (!$this->logged_in) {
			$this->username = $_SESSION['username'] = _GUEST;
	  ...
	  {
          if (isset($_SESSION['username']) && $_SESSION['username'] != _GUEST) {
	  ...
	  public function verifyCustomFields($type)
	  {
		  ...
		  if ($fdata = $db->fetch_all("SELECT * FROM custom_fields WHERE type = '" . $db->escape($type) . "' AND active = 1 AND req = 1")) {

			  $res = '';
			  foreach ($fdata as $cfrow) {
				  if(empty($_POST['custom_' . $cfrow['name']]))
					$res .= $core->msgs['custom_' . $cfrow['name']] = _CFL_ENTER_F . $cfrow['title' . $core->dblang];
			  }
			  return $res;
			  unset($cfrow);
     ...
     private function validateToken($token)
	  ...
	  public function getPermissionList($access = false)
	  {
		  ...
		  $plugdata = $db->fetch_all("SELECT title{$core->dblang}, plugalias FROM plugins");
