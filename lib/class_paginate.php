          ...
          if ($this->num_pages > 1) {
              ...
              for ($i = 1; $i <= $this->num_pages; $i++) {
                  if ($this->range[0] > 2 && $i == $this->range[0])
                      $this->display .= "<a href=\"#\" class=\"no-more\"> ... </a>";
                  ...
                  if ($i == 1 or $i == $this->num_pages or in_array($i, $this->range)) {
                      if ($i == $this->current_page) {
                          ...
                          if ($path && $core->seo) {
                              $this->display .= "<a class=\"tooltip number\" title=\"" . _PAG_GOTO . " $i " . _PAG_OF . " $this->num_pages\" href=\"".$path.$i."-".$this->items_per_page.".html\">$i</a>";
                          } else {
                              $this->display .= "<a class=\"tooltip number\" title=\"" . _PAG_GOTO . " $i " . _PAG_OF . " $this->num_pages\" href=\"$_SERVER[PHP_SELF]?pg=$i&amp;ipp=$this->items_per_page$this->querystring\">$i</a>";
                          }
                      }
                  }
