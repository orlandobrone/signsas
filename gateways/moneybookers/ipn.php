<?php
  ...
  if (intval($_POST['status']) == 2) {
	  ...
	  */
	  ...
	  array($username, $row['title'.$core->dblang], $core->formatMoney($mc_gross), "Completed", "MoneyBookers"), $row2['body'.$core->dblang]);
      ...
      $mailer->send($message);
      $wojosec->writeLog($username . ' ' . _LG_PAYMENT_OK . ' ' . $row['title'], "", "yes", "payment");
  } else {
      ...
      $body = str_replace(array('[USERNAME]', '[ITEMNAME]', '[PRICE]', '[STATUS]', '[PP]'), 
	  array($username, $itemname, $core->formatMoney($mc_gross), "Failed", "MoneyBookers"), $row2['body'].$core->dblang);
