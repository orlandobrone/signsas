    ...
    <table class="forms">
      <tr>
        <td style="width:250px; vertical-align:top;"><?php if ($pluginrow == 0): ?>
          ...
          <?php else:?>
          <ul id="default-<?php echo ($modid) ? $modid : $pageid;?>" class="modList default">
            <?php foreach ($pluginrow as $mrow): ?>
            <li id="list-<?php echo $mrow['id'];?>"><?php echo $mrow['title'.$core->dblang];?><span></span></li>
            <?php endforeach; ?>
            <?php unset($mrow);?>
          </ul>
          <?php endif;?></td>
        <td><table id="layout">
            <tr>
              <td colspan="3" class="top-position"><ul id="top-<?php echo ($modid) ? $modid : $pageid;?>" class="modList">
                  ...
                  <li id="list-<?php echo $trow['plid'];?>"><?php echo $trow['title'.$core->dblang];?><span><a href="javascript:void(0);" id="spc-<?php echo $trow['plid'];?>" class="plugspace" data-space="<?php echo $trow['plid'];?>" data-cspace="<?php echo $trow['space'];?>"><img src="images/config-small.png" alt=""/></a></span></li>
                  <?php endif; ?>
                  <?php endforeach; ?>
                  <?php unset($trow);?>
                  <?php endif; ?>
            ...
            <tr>
              <td colspan="3" class="bottom-position"><ul id="bottom-<?php echo ($modid) ? $modid : $pageid;?>" class="modList">
                  ...
                  <li id="list-<?php echo $brow['plid'];?>"><?php echo $brow['title'.$core->dblang];?><span><a href="javascript:void(0);" id="spc-<?php echo $brow['plid'];?>" class="plugspace" data-space="<?php echo $brow['plid'];?>" data-cspace="<?php echo $brow['space'];?>"><img src="images/config-small.png" alt=""/></a></span></li>
                  <?php endif; ?>
                  <?php endforeach; ?>
                  <?php unset($brow);?>
                  <?php endif; ?>
...
<?php endif;?>
$(function() {
    $(".top-position, .bottom-position").on("sortreceive", function (event, ui) {
		var id = $(ui.item).attr('id').replace('list-', '')
        $('span:last-child', ui.item).append('<a href="javascript:void(0);" id="spc-' + id + '" class="plugspace" data-space="' + id + '" data-cspace="-1"><img src="images/config-small.png" alt=""/></a>');
    });
    $(".default").on("sortreceive", function (event, ui) {
        $('a:last-child', ui.item).remove();
    });
	...
	$('#layout').on('click', 'a.plugspace', function () {		
		var space = $(this).attr("data-space");
		var curspace = $(this).attr("data-cspace");
		var id = $(this).attr('data-space');
        var text = "<div><?php echo _LY_SUB;?>: <input name=\"total-space\" type=\"text\"  size=\"5\" id=\"total-space\" value=\"" + curspace + "\" class=\"inputbox-sml\"/></div>";
		$.confirm({
			title: '<?php echo _LY_SPTITLE;?>',
			message: text,
            buttons: {
                <?php echo _SUBMIT;?>: {
                    class: 'yes',
                    action: function () {
                        $.ajax({
                            ...
                            data: {
                                'setspace': space,
								'type': '<?php echo isset($_GET['module']) ? 'module' : 'page';?>',
								'ptype': '<?php echo ($modid) ? $modid : $pageid;?>',
                                'newspace': $("#total-space").val()
                            },
                            success: function (res) {
                                    $("a#spc-" + id).attr("data-cspace", res);
                            }
                        });
                    }
                },
