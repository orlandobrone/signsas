...
<?php
  ...
  if (isset($_POST['setspace'])):
     $plug_id = intval($_POST['setspace']);
	 $id = intval($_POST['ptype']);
	 $type = sanitize($_POST['type']);
	 $ptype = ($type == "page" ? "page_id" : "mod_id"); 
	 
	 $newspace = intval($_POST['newspace']);
	 $data = array(
			  'space' => $newspace, 
	 );
     $db->update("layout", $data, "plug_id = $plug_id AND $ptype = $id");
	 print $newspace;
  endif;
?>
<?php
...
<?php
  ...
  if (isset($_POST['deleteField']))
      ...
      : redirect_to("index.php?do=fields");
  endif;
  ...
  print ($db->affected()) ? $wojosec->writeLog(_CFL_FIELD .' <strong>'.$title.'</strong> '._DELETED, "", "no", "content") . $core->msgOk(_CFL_FIELD .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>
<?php
  /* Delete SQL Backup */
