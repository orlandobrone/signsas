...
<?php
  ...
  if (isset($_POST['deleteSlide'])):
	...
	@unlink(WOJOLITE . 'plugins/elastic/slides/'.$filename);
	$db->delete("plug_elastic", "id='" . $id . "'");
	
	$title = sanitize($_POST['title']);
	print ($db->affected()) ? $wojosec->writeLog(PLG_JQE_SLIDE .' <strong>'.$title.'</strong> '._DELETED, "", "no", "plugin") . $core->msgOk(PLG_JQE_SLIDE .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>