...
<?php
  ...
  if (isset($_POST['deleteSlide'])):
	...
	@unlink(WOJOLITE . 'plugins/slideout/slides/'.$filename);
	$db->delete("plug_slideout", "id='" . (int)$id . "'");
	
	$title = sanitize($_POST['title']);
	print ($db->affected()) ? $wojosec->writeLog(PLG_SO_SLIDE .' <strong>'.$title.'</strong> '._DELETED, "", "no", "plugin") . $core->msgOk(PLG_SO_SLIDE .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>