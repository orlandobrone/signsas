<?php
  ...
  class eventManager
  {
      ...
      private function getCalDataMonth()
      {
		  ...
		  $sql = "SELECT e.*, e.id as event_id, ed.id as eid, DAY(event_date) as sday, title{$core->dblang} as etitle, DAY(date_end) as eday, ed.color,"
		  . "\n DATE_FORMAT(date_start,'%d %M %Y') AS full_date,"
		  . "\n DATE_FORMAT(time_start,'%H:%i') AS stime,"
		  . "\n DATE_FORMAT(time_end,'%H:%i') AS etime"
		  . "\n FROM " . $this->mTable . " as e"
		  . "\n LEFT JOIN " . $this->dTable . " as ed ON ed.event_id = e.id" 
			  ...
			  for ($k = $max_days + 1; $k <= 7; $k++) {
				  ...
				  if ($this->checkEventsMonths($is_day)) {
					  ...
					  foreach ($this->eventMonth as $row) {
						  if ($row['sday'] == $is_day) {
							  $res .= "<div><a title=\"" . $row['full_date'] . "\" href=\"" . SITEURL . "/modules/events/controller.php?eventid=" . $row['event_id'] . "\" class=\"ajax\" style=\"color:#" . $row['color'] . "\">" . character_limiter($row['etitle'],15) . "</a></div>";
						  }
					  }
					  $display = $data . $is_day;
					  $class = " content";
		  ...
		  for ($i = 0; $i < $fullWeeks; $i++) {
			  ...
			  for ($j = 0; $j < 7; $j++) {
				  ...
				  if ($this->checkEventsMonths($is_day)) {
					  ...
					  foreach ($this->eventMonth as $row) {
						  if ($row['sday'] == $is_day) {
							  $res .= "<div><a title=\"" . $row['full_date'] . "\" href=\"" . SITEURL . "/modules/events/controller.php?eventid=" . $row['event_id'] . "\" style=\"color:#" . $row['color'] . "\" class=\"ajax\">" . character_limiter($row['etitle'],15) . "</a></div>";
						  }
					  }
					  $display = $data . $is_day;
					  $class = " content";
		  ...
		  if ($max_days < 7) {
			  ...
			  for ($i = $max_days + 1; $i <= 7; $i++) {
				  ...
				  if ($this->checkEventsMonths($is_day)) {					  
					  ...
					  $display = $data . "<a href=\"" . SITEURL . "/plugins/events/controller.php?d=$is_day&amp;m=$m&amp;y=$datayear\" class=\"ajax\" title=\"$is_day $datamonth $datayear\">" . $is_day . "</a>";
					  $class = " events";
				  } else {
					   $display = $is_day;
				  }
